...
function MatchTab({ preset }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {toast && (
        <div className="flex items-start gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 animate-fade-up">
          <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-emerald-600 text-white"><Icon name="check" size={16} /></span>
          ...
          <Card key={mm.orgId} className="p-4">
            ...
            <div className="mt-3 flex items-center gap-2 border-t border-edge pt-3">
              ...
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-mist-700/50">
                <div className={cx('h-full rounded-full', mm.total >= 70 ? 'bg-emerald-500' : mm.total >= 50 ? 'bg-brand-500' : 'bg-amber-400')} style={{ width: mm.total + '%' }} />
              </div>
                ...
                <div className="space-y-2">
                  {(t.custody || []).map((c, i) => (
                    <div key={i} className="flex gap-3">
                      <span className="grid size-5 shrink-0 place-items-center rounded-full bg-emerald-500/15 text-emerald-300 ring-1 ring-emerald-500/30"><Icon name="check" size={10} /></span>
                      <div>

...
import { Card, Pill, ScoreBar, Drawer, SectionTitle, Chip, fmtNum, Button, Icon, cx } from '../components/ui.jsx';
  ...
  return (
    <div className="animate-fade-up space-y-4">
      <div className="border-b border-edge pb-4">
        <h2 className="text-base font-semibold text-mist-50">Inventory</h2>
        <p className="mt-0.5 text-[11px] text-mist-400">{rows.length} batches · {counts.atrisk} at risk · {counts.surplus} surplus</p>
      </div>
...
function BatchDetail({ b, analysis: a, holder, mfg, onMatch }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="border border-edge bg-panel2 p-4">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-mist-400">Surplus formula</p>
        <div className="mt-2 flex flex-wrap items-center gap-1.5 text-xs font-medium text-mist-200">
          {formula.map((f, i) => (
            <span key={i} className="flex items-center gap-1.5">
              {f.op && <span className="text-mist-500">{f.op}</span>}
              <span className="border border-edge bg-panel px-2 py-1 rounded">
                {f.label === 'Days to expiry' ? `${f.value} d` : fmtNum(f.value)}
              </span>
            </span>
          ))}
          <span className="text-mist-500">=</span>
          <span className={a.projectedSurplus > 0 ? 'border border-brand-500/30 bg-brand-500/10 px-2 py-1 text-brand-300 rounded font-semibold' : 'border border-edge bg-panel px-2 py-1 text-mist-400 rounded'}>
      ...
      {a.surplus && (
        <div className="flex items-center justify-between gap-3 border border-brand-500/30 bg-brand-500/5 p-4">
          <div>
            <p className="text-xs font-semibold text-mist-100">Rescuable surplus — {fmtNum(a.projectedSurplus)} units</p>
